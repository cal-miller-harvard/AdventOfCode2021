...
function solve(input)
    function parsePacket(p)
        version = parse(Int, p[1:3]; base=2)
        ...
        if type == 4
            ...
            return ((version, type, parse(Int, s; base=2)), remainder)
        else
            length_type = p[7]
            if length_type == '0'
                ...
                return ((version, type, parsedSubpackets), remainder)
            else
    ...
    return getVersion(parsePacket(input)[1])
end
